...
| Live quote REST | Implemented | Quote snapshot includes LTP, volume, bid/offer, OI, OI change and last-trade time mapping | Strict required LTP/timestamp validation; per-segment nullability still needs empirical validation |
...
| Feed | Partial | Official Python feed docs describe subscriptions up to 1000 instruments, market streams, equity/F&O order updates, and derivatives position updates | Protocol, reconnect/replay, sequence, heartbeat, .NET support, quotas, and source timestamps need confirmation; do not port SDK internals blindly |
