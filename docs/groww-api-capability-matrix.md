...
Re-reviewed against the official Groww Trading API cURL and feed documentation on **2026-08-03**. Profile, quote, non-deprecated historical candles, and instrument CSV are implemented behind a read-only contract with sanitised fake-response tests. Every later endpoint must still be rechecked immediately before implementation.
...
| Option chain | Supported | Official `GET /v1/option-chain/exchange/{exchange}/underlying/{underlying}` with expiry date; response contains underlying LTP, CE/PE LTP, OI, volume and Greeks | Captured once per minute for nearest Nifty and Sensex expiry; provider does not expose a snapshot timestamp |
| Instrument master | Implemented, partial lifecycle | Public downloadable CSV with exchange token, Groww/trading symbols, expiry, strike, lot/tick metadata and segments | Atomic Nifty index/future/option upsert implemented; official Groww symbols are persisted for historical calls; checksum/version and safe deactivation remain unresolved |
...
| Live quote REST | Implemented for Nifty spot, nearest future and selected option | Quote snapshot includes LTP, volume, bid/offer, OI, OI change and last-trade time mapping | Spot drives price levels; nearest-future volume/OI confirms signals; selected-option depth validates simulated execution. Per-segment nullability still needs paper-soak evidence |
...
| Historical candles (new) | Implemented with session bootstrap | `/v1/historical/candles`; OHLC, volume, F&O OI; 1–5 minute requests allow up to 30 days; data from 2020 stated | Seven-field strict mapping, IST-to-UTC conversion, idempotent import, and seven-day spot/future bootstrap implemented; exchange-calendar gap classification remains unresolved |
